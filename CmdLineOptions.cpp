...
bool CmdLineOptions::FillFromArgs(int argc, char** argv)
{
    try
    {
        auto checkDepthValue = [](const __int8& value)
            {
        ...
        desc.add_options()
            ...
            ("depth,d", po::value<__int8>()->default_value(0)->notifier(checkDepthValue), "Scan depth, 1 - all directories, 0 - current folder only. Default value is 0")
        ...
        _scanLevel = ScanLevel(vm["depth"].as<__int8>());
